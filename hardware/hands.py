...
from hardware.lib_robotis_mod import *
import time
			...
			for i in range(num_servos):
				...
				if series == "RX":
					if (self.motorDir[i] ==1 and enc > 512) or (self.motorDir[i] == -1 and enc < 512):
						...
						inp = input("Your encoder position for motor index " + str(i) + " may cause the motor to move backwards and break gears. We recommend you resetting the fingers to prevent gear shear, proceed? [ENTER]")
					else:
				...
				#These would then be the MX and XM motors
					...
					print( "Failsafe is incorporated to prevent gear shear in Model O")
					print( "Motor encoder postion: ", enc)
					inp = input("As an XM Motor, we can automatically fix this issue for motor " + str(i) + ", proceed? [ENTER]")
